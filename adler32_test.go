...
func BenchmarkThisLarge(b *testing.B) {
	data := generateRandomByte(1000000)
	b.ResetTimer()
	for n := 0; n < b.N; n++ {
		Checksum(data)
	}
...
func BenchmarkStdLibLarge(b *testing.B) {
	data := generateRandomByte(1000000)
	b.ResetTimer()
	for n := 0; n < b.N; n++ {
		adler32.Checksum(data)
...
func BenchmarkThisExtraLarge(b *testing.B) {
	data := generateRandomByte(10000000)
	b.ResetTimer()
	for n := 0; n < b.N; n++ {
		Checksum(data)
	}
...
func BenchmarkStdLibExtraLarge(b *testing.B) {
	data := generateRandomByte(10000000)
	b.ResetTimer()
	for n := 0; n < b.N; n++ {
		adler32.Checksum(data)
